...
class MetadataConditionEncoder(nn.Module):
    def __init__(
        ...
        for cfg in field_configs:
            ...
            if cfg["type"] == "categorical":
                ...
                raise ValueError(f"Unknown field type '{cfg['type']}' for field '{name}'.")
        ...
        if n_padding < 0:
            raise ValueError(
                f"Too many fields ({len(field_configs)}) — exceeds seq_len={seq_len}."
            )
    ...
    def forward(self, meta_batch: dict) -> torch.Tensor:
        ...
        for cfg in self.field_configs:
            ...
            embs[name] = emb
            tokens.append(self.projections[name](emb).unsqueeze(1))
        ...
        if self.n_padding > 0:
            tokens.append(self.padding_tokens.expand(B, -1, -1))

        cond = torch.cat(tokens, dim=1)
        assert cond.shape == (B, self.seq_len, self.final_dim)
        return cond

def awa2_field_configs(dataset, class_emb_dim: int = 32) -> list:
    configs = [
        {
            "name":       "class",
            "type":       "categorical",
            "vocab_size": len(dataset.class2idx),
