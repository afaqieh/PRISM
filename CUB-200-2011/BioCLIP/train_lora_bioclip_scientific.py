...
OUT_DIR     = "results/cub_stage1_bioclip_scientific"
STEPS       = 40000
BATCH_SIZE  = 4
LR          = 3e-5
LORA_RANK   = 16
...
def save_sample(unet, vae, bioclip_model, tokenizer, projection,
                out_dir, device, step, species2idx, df):
    ...
    for species_name in sorted(species2idx.keys()):
        row    = df[df["species_name"] == species_name].sample(
...
def train():
    ...
    dataset      = CUBDatasetPrompt(TRAIN_CSV, vae, mode="prompt", device=device, prompt_fn=create_prompt_bioclip)
    class_counts = dataset.df["species_name"].value_counts().to_dict()
    weights      = dataset.df["species_name"].map(
    ...
    torch.save(
        ...
        os.path.join(OUT_DIR, "lora_bioclip_cub.pth"),
    )
    print(f"Done {OUT_DIR}/lora_bioclip_cub.pth")
