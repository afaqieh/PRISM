...
SAVE_PATH   = "dino_linear_classifier_plantvillage.pth"
...
def main():
    ...
    if not os.path.exists(args.train_csv) or not os.path.exists(args.val_csv):
        raise FileNotFoundError(
            f"Split CSVs not found. Run create_split_plantvillage.py first.\n"
            f"  Expected: {args.train_csv}\n"
            f"            {args.val_csv}"
        )
    ...
    for c in classes:
        print(f"  - {c}  ->  {prettify_class_name(c)}")
    ...
    train_dataset = PlantVillageDataset(train_df, class2idx, train_transform)
    val_dataset   = PlantVillageDataset(val_df,   class2idx, val_transform)
    ...
    for epoch in range(EPOCHS):
        ...
        for c in range(num_classes):
            ...
            per_cls.append((all_preds_np[mask] == c).mean() if mask.sum() > 0 else 0.0)

        short_classes = [prettify_class_name(name) for name in classes]
        ...
        if bal_acc > best_val_acc:
            ...
            torch.save({
                ...
                "class2idx":         class2idx,
                "idx2class":         idx2class,
                "display_names":     {c: prettify_class_name(c) for c in classes},
                "val_acc":           val_acc,
                "bal_acc":           bal_acc,
                "epoch":             epoch + 1,
            }, args.save_path)
    ...
    short_labels = [prettify_class_name(name) for name in classes]
    ...
    ax.set_title(
        f"DINOv2 linear probe ({args.dino_model}) — confusion matrix on val set\n"
    ...
    print(f"\nNow run evaluate_dino_plantvillage.py — it will load these weights automatically.")
